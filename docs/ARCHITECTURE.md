...
- `ActivityRadar`: menu-bar and command-palette UI, local preferences,
  ...
  boundary.
- `ActivityRadarDiagnostics`: aggregate, fixed-schema compatibility report with
  no task text or raw task identifiers.
...
4. A `codex://` URL is handed to macOS only after a user requests a task handoff.
   An accepted OS request is recorded, but final Codex navigation is not observed.
5. Dashboard analysis is offline and starts no background agent call.
6. Opening Wingman does not invoke Codex CLI. A separate explicit compatibility
   check runs version, command, and login-status checks without an agent turn or
   task packet, using the documented private temporary authentication copy.
7. The optional remote Wingman review is a distinct, user-triggered boundary:
   AiWingman makes the exact user-derived JSON packet it intends to supply available for inspection,
...
8. AiWingman requests Codex CLI read-only sandbox mode, intended to deny agent-
   tool writes to the workspace. This is neither OS-level isolation nor a zero-
   filesystem-write guarantee, and it does not prove that no other local file
   can be read. The previewed packet is not a sole-context guarantee.
9. The saved Codex authentication file is copied opaquely into a private
...
parent/child forest from one read transaction. Spawned rollout files may share a
cumulative token-counter lineage, so root and descendant counters are not
assumed additive. AiWingman ranks each tree by its largest observed cumulative
